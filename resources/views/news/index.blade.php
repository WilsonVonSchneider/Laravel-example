    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div id="form-container" class="hidden">
                <form class="flex items-center mb-8 w-full" action="/news">
                    ...
                    <input type="date" name="toDate" id="toDate"
                        class="w-full mr-4 rounded-lg focus:outline-none focus:shadow text-gray-500"
                        max="<?php echo date('Y-m-d'); ?>" min="<?php echo date('Y-m-d', strtotime('-30 days')); ?>">
                    <input type="text" name="domains" placeholder="Domains: {{ $domains }}"
                        class="mr-4 w-full rounded-lg focus:outline-none focus:shadow text-gray-500">
                    <select name="sortBy" id="sortBy"
                        class="mr-4 rounded-lg focus:outline-none focus:shadow text-gray-500">
                        <option value="none" selected disabled hidden>Sort by: {{ $sortBy }}</option>
                        @foreach ($allSortBy as $sort)
            ...
            <div class="bg-gray-100 rounded-lg shadow-md p-6">
                <div>
                    <h2 class="font-bold mb-4">NEWS</h2>
                        ...
                        <div class="mb-6">
                            ...
                            @if (array_search($newTop->url, $arrayOfMatchedUrls))
                                <div class="text-gray-600"><span
                                        class="mt-5 font-xl fa fa-star checked text-yellow-400">&#160&#160</span> Added
                                    to favorites</div>
                            @else
                            ...
                            <div class="mt-5">
                                <div class="mt-5">
                                    <form class="mb-4 flex flex-row" method="POST" action="{{ route('comments.store') }}">
                                        ...
                                        <input type="hidden" id="url" name="url"
                                            value="{{ $newTop->url }}">
                                        <button type="submit"
                                            class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-400">Submit</button>
                                    </form>
                                </div>
                                ...
                                <h3 class="text-lg font-bold mb-5 text-gray-600"><button
                            ...
                            @if ($numberOfPages > 1)
                                <div class="flex flex-wrap justify-center">
                                    @php
                                        ...
                                        <a href="{{ $url }}?page=1&q={{ $q }}&fromDate={{ $fromDate }}&toDate={{ $toDate }}&domains={{ $domains }}&sortBy={{ $sortBy }}&source={{ $source }}"
                                    ...
                                    @for ($x = $startPage; $x <= $endPage; $x++)
                                        <a href="{{ $url }}?page={{ $x }}&q={{ $q }}&fromDate={{ $fromDate }}&toDate={{ $toDate }}&domains={{ $domains }}&sortBy={{ $sortBy }}&source={{ $source }}"
                                    ...
                                    @if ($endPage < $numberOfPages)
                                        @if ($endPage < $numberOfPages - 1)
                                            <span class="mx-1 px-2 py-1 rounded-lg bg-gray-300 text-sm font-medium">
                                                ...</span>
                                        @endif
                                        <a href="{{ $url }}?page={{ $numberOfPages }}&q={{ $q }}&fromDate={{ $fromDate }}&toDate={{ $toDate }}&domains={{ $domains }}&sortBy={{ $sortBy }}&source={{ $source }}"
